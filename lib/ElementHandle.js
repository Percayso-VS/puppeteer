...
class ElementHandle extends JSHandle {
  ...
  async click(options = {}) {
    await this.focus();
    const {x, y} = await this._visibleCenter();
    let xVariation = ( x /100 ) * 0.5
    let xLowercap = x + xVariation
    let xUppercap = x - xVariation
    let yVariation = ( y /100 ) * 0.5
    let yLowercap = y + yVariation
    let yUppercap = y - yVariation

    await this._page.mouse.move(Math.floor(Math.random() * xUppercap) + xLowercap, Math.floor(Math.random() * yUppercap) + yLowercap);
    await this._page.waitFor(Math.floor(Math.random()*100))
   ...
   */
  async type(text, options, type) {
    await this.focus();
    await this._page.keyboard.type(text, options, type, this);
  }
